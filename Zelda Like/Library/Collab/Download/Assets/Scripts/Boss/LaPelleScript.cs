...
public class LaPelleScript : MonoBehaviour {
	...
	private int fightStart = 0; // a 0, le combat n'a pas commencé
	...
	void Update () {
		...
		if (fightStart == 1)  //Vrai lorsque le combat est en train d'avoir lieu.
		{
	...
	void OnTriggerEnter2D (Collider2D other)
	{
		...
		if (other.tag == "playerAttack" && target.GetComponent<PlayerController>().isAttacking )
		{
			BossTakeDamage();
		}
	}
